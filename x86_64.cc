...
static void write_uint32(Bytes *buffer, uint32_t value) {
...
static void write_uint64(Bytes *buffer, uint64_t value) {
  bytes_add(buffer, (value >> 0) & 0xFF);
  bytes_add(buffer, (value >> 8) & 0xFF);
  bytes_add(buffer, (value >> 16) & 0xFF);
  bytes_add(buffer, (value >> 24) & 0xFF);
...
void x86_64_mov32_reg(Bytes *buffer, int reg1, int reg2) {
...
void x86_64_mov64_reg(Bytes *buffer, int reg1, int reg2) {
  bytes_add(buffer, ADDRESS_64_PREFIX);
  bytes_add(buffer, 0x89);
  bytes_add(buffer, 0xC0 | (reg1 << 3) | reg2);
}
...
void x86_64_push64(Bytes *buffer, int reg) { bytes_add(buffer, 0x50 + reg); }
...
void x86_64_pop64(Bytes *buffer, int reg) { bytes_add(buffer, 0x58 + reg); }
